...
s의 길이가 1과 같지 않고, 1보다 큰 경우 동안 아래 과정을 반복하도록 한다.   
1. 이진변환의 횟수를 저장하기 위해 trans변수에 저장하고, 제거된 0의 개수를 세기 위해 count()함수를 사용하여 zeros변수에 저장한다.   
2. 모든 0을 제거하기 위해 replace()함수를 사용하여 “0”을 “”로 대체해준다.   
3. 제거 후 문자열의 길이를 다시 bin()함수를 사용하여 이진수로 변환해준다 bin()함수는 변환한 이진수 앞에 `0b`가 붙기 때문에 슬라이싱을 통해 `0b` 를 제거해준다.   
</br>
...
```python
bin_str = s.replace("0", "") # 문자열 전체에서 "0"을 ""로 변경(삭제)
bin_str = s.replace("0", "2", 1) # 문자열에서 첫번째 "0"을 "2"로 변경
```
</br>

### bin()
bin(), oct(), hex()는 파이썬에서 제공하는 내장함수로, 각각 이진수, 8진수, 16진수로 변환해준다. 변환 후 각각 앞에 “0b”, “0o”, “0x”가 붙는다.      
따라서 변환된 값만 얻을 것이라면, 슬라이싱 처리를 해주면 된다.     
```python
s = bin(size)[2:] # 2번째 인덱스부터 s에 저장 (인덱스 0과 1은 버림)
